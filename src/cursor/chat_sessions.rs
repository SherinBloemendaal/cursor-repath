...
fn load_global_registry_sessions(
    ...
    if crate::cursor::registry::composer_headers_table(conn)? {
        let workspace_id = identity.workspace_id.as_deref().unwrap_or("");
        let headers = crate::cursor::registry::load_headers_for_workspace(conn, workspace_id)?;
        return Ok(headers
            .into_iter()
            .filter(|header| include_archived || !header.is_archived)
            .map(|header| SessionMetadata {
                composer_id: header.composer_id,
                title: header.title,
                created_at_ms: header.created_at,
                updated_at_ms: header.last_updated_at.or(header.created_at),
            })
            .collect());
    }
